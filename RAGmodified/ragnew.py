...
# -----------------------------
# Build Retriever (UPDATED FOR STEP 1)
# -----------------------------
...
# Check if both FAISS vector index and pickle metadata exist
if index_file.exists() and pkl_file.exists():
    print(f"Loading existing FAISS index from: {index_directory}")
    vectorstore = FAISS.load_local(
        folder_path=str(index_directory),
        embeddings=embeddings,
        allow_dangerous_deserialization=True  # Needed to load local pickle files safely
    )
    # Extract existing chunks from docstore so you have them for Step 2 (BM25)
